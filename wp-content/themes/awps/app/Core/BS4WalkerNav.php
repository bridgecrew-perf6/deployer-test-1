...
/**
 *
 * Bootstrap 4 WordPress navbar walker menu
wp_nav_menu( [
	...
	'fallback_cb'     => 'BS4WalkerNav::fallback',
	'walker'          => new BS4WalkerNav(),
] );
 */
...
class BS4WalkerNav extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
		 ...
		 */
		$classes = $this->separate_linkmods_and_icons_from_classes( $classes, $linkmod_classes, $icon_classes,
			$depth );
	...
	public static function fallback( $args ) {
		...
		$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu',
		...
		if ( isset( $args[ 'items_wrap' ] ) ) {
			...
			if ( strpos( $wrap, 'SiteNavigationElement' ) === false ) {
				$args[ 'items_wrap' ] = preg_replace( '/(>).*>?\%3\$s/',
					' itemscope itemtype="http://www.schema.org/SiteNavigationElement"$0', $wrap );
			}
		}
